...
package com.util.date.thirdparty;
 ...
 *
 * @since 3.6 (https://github.com/apache/commons-lang/tree/LANG_3_6)
 */
public class FastDateParser implements DateParser, Serializable {
    ...
    private final int mCentury;
    private final int mStartYear;

    // derived fields
    private transient List<StrategyAndWidth> mPatterns;
    ...
    protected FastDateParser(final String pattern, final TimeZone timeZone, final Locale locale,
        ...
        mCentury = centuryStartYear / 100 * 100;
        mStartYear = centuryStartYear - mCentury;
    ...
    private static class StrategyAndWidth {
        ...
        int getMaxWidth(final ListIterator<StrategyAndWidth> lt) {
            if (!mStrategy.isNumber() || !lt.hasNext()) {
                return 0;
            }
            final Strategy nextStrategy = lt.next().mStrategy;
            lt.previous();
            return nextStrategy.isNumber() ? mWidth : 0;
        }
    }
    ...
    private class StrategyParser {
        private final Calendar mDefiningCalendar;
        private int mCurrentIdx;

        StrategyParser(final Calendar definingCalendar) {
            this.mDefiningCalendar = definingCalendar;
        }

        StrategyAndWidth getNextStrategy() {
            if (mCurrentIdx >= mPattern.length()) {
                return null;
            }

            final char c = mPattern.charAt(mCurrentIdx);
            if (isFormatLetter(c)) {
        ...
        private StrategyAndWidth letterPattern(final char c) {
            final int begin = mCurrentIdx;
            while (++mCurrentIdx < mPattern.length()) {
                if (mPattern.charAt(mCurrentIdx) != c) {
                    break;
                }
            }

            final int width = mCurrentIdx - begin;
            return new StrategyAndWidth(getStrategy(c, width, mDefiningCalendar), width);
        }
        ...
        private StrategyAndWidth literal() {
            ...
            while (mCurrentIdx < mPattern.length()) {
                ...
                if (!activeQuote && isFormatLetter(c)) {
                    break;
                } else if (c == '\'' && (++mCurrentIdx == mPattern.length() || mPattern.charAt(mCurrentIdx) != '\'')) {
    ...
    public String toString() {
        return "FastDateParser[" + mPattern + "," + mLocale + "," + mTimeZone.getID() + "]";
    }
    ...
    private void readObject(final ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        final Calendar definingCalendar = Calendar.getInstance(mTimeZone, mLocale);
        init(definingCalendar);
    }
    ...
    public Date parse(final String source) throws ParseException {
        ...
        if (date == null) {
            ...
            if (mLocale.equals(JAPANESE_IMPERIAL)) {
                throw new ParseException(
                        "(The " + mLocale + " locale does not support dates before 1868 AD)\n"
    ...
    public Date parse(final String source, final ParsePosition pos) {
        ...
        final Calendar cal = Calendar.getInstance(mTimeZone, mLocale);
        cal.clear();
    ...
    public boolean parse(final String source, final ParsePosition pos, final Calendar calendar) {
        final ListIterator<StrategyAndWidth> lt = mPatterns.listIterator();
        while (lt.hasNext()) {
            final StrategyAndWidth strategyAndWidth = lt.next();
            final int maxWidth = strategyAndWidth.getMaxWidth(lt);
            if (!strategyAndWidth.mStrategy.parse(this, calendar, source, pos, maxWidth)) {
                return false;
            }
        }
        return true;
    ...
    private int adjustYear(final int twoDigitYear) {
        final int trial = mCentury + twoDigitYear;
        return twoDigitYear >= mStartYear ? trial : trial + 100;
    }
    ...
    private abstract static class PatternStrategy extends Strategy {
        ...
        boolean parse(final FastDateParser parser, final Calendar calendar, final String source,
            ...
            final Matcher matcher = mPattern.matcher(source.substring(pos.getIndex()));
    ...
    private Strategy getLocaleSpecificStrategy(final int field, final Calendar definingCalendar) {
        ...
        Strategy strategy = cache.get(mLocale);
        if (strategy == null) {
            strategy = field == Calendar.ZONE_OFFSET
                    ? new TimeZoneStrategy(mLocale)
                    : new CaseInsensitiveTextStrategy(field, definingCalendar, mLocale);
            final Strategy inCache = cache.putIfAbsent(mLocale, strategy);
            if (inCache != null) {
                return inCache;
            }
        }
    ...
    private static class CopyQuotedStrategy extends Strategy {
        ...
        private final String mFormatField;
        ...
        boolean parse(final FastDateParser parser, final Calendar calendar, final String source,
                final ParsePosition pos, final int maxWidth) {
            for (int idx = 0; idx < mFormatField.length(); ++idx) {
                final int sIdx = idx + pos.getIndex();
                if (sIdx == source.length()) {
                    pos.setErrorIndex(sIdx);
                    return false;
                }
                if (mFormatField.charAt(idx) != source.charAt(sIdx)) {
                    pos.setErrorIndex(sIdx);
                    return false;
                }
            }
            pos.setIndex(mFormatField.length() + pos.getIndex());
            return true;
        }
    }
    ...
    private static class CaseInsensitiveTextStrategy extends PatternStrategy {
        ...
        private final Map<String, Integer> mLKeyValues;
        ...
        CaseInsensitiveTextStrategy(final int field, final Calendar definingCalendar, final Locale locale) {
            ...
            regex.append("((?iu)");
            mLKeyValues = appendDisplayNames(definingCalendar, locale, field, regex);
        ...
        void setCalendar(final FastDateParser parser, final Calendar cal, final String value) {
            final Integer iVal = mLKeyValues.get(value.toLowerCase(mLocale));
            cal.set(mField, iVal.intValue());
        }
    }
    ...
    private static class NumberStrategy extends Strategy {
        ...
        boolean parse(final FastDateParser parser, final Calendar calendar, final String source,
            ...
            calendar.set(mField, modify(parser, value));
            return true;
        }
    ...
    static class TimeZoneStrategy extends PatternStrategy {
        ...
        private final Locale mLocale;
        private final Map<String, TzInfo> mTzNames = new HashMap<>();
        ...
        private static class TzInfo {
            ...
            TzInfo(final TimeZone tz, final boolean useDst) {
                mZone = tz;
                mDstOffset = useDst ? tz.getDSTSavings() : 0;
            }
        }
        ...
        void setCalendar(final FastDateParser parser, final Calendar cal, final String value) {
            if (value.charAt(0) == '+' || value.charAt(0) == '-') {
                ...
                final TzInfo tzInfo = mTzNames.get(value.toLowerCase(mLocale));
                cal.set(Calendar.DST_OFFSET, tzInfo.mDstOffset);
                cal.set(Calendar.ZONE_OFFSET, tzInfo.mZone.getRawOffset());
            }
        }
    }
